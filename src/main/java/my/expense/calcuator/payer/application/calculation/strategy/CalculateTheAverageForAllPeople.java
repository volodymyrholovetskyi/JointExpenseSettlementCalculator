package my.expense.calcuator.payer.application.calculation.strategy;

import my.expense.calcuator.payer.application.calculation.CalculateExpenseStrategy;
import my.expense.calcuator.payer.domain.Payer;

import java.math.BigDecimal;
import java.util.List;

public class CalculateTheTotalCostForAllPeople implements CalculateExpenseStrategy {

    @Override
    public void calculate(BigDecimal averages, List<Payer> payers) {


    }
}
